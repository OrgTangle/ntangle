...
been attempted multiple times with disputable success.  But there's an
easy way: all it takes to understand Git internals is to reimplement
Git from scratch.

No, don't run.

I'm really serious: if you read this article top to bottom and write
the code (or just clone [[https://github.com/thblt/write-yourself-a-git][the repository]] --- but you should write the
code yourself, really), you'll end up with a program, called =wyag=,
that will implement all the fundamental features of git: =init=, =add=,
=rm=, =status=, =commit=, =log=... in a way that is perfectly compatible with
=git= itself.  The last commit of this article was actually created with
=wyag=, not =git=.

But isn't Git too complex for such an approach?  That Git is complex
is, in my opinion, a misconception.  Git is a large program, with a
lot of features, that's true.  But the core of that program is
actually extremely simple.  I think it's apparent complexity comes
from two facts.  First, it's often deeply counterintuitive (when we'll
discuss commits, we'll notice that for a commit, time flows from the
future to the past![fn:1]).  But most importantly, there's this other
fact that its core model is extremely simple while being extremely
powerful.  The combination of core simplicity and powerful
applications often makes thing really hard to grasp, because of the
mental jump required to derive the variety of applications from the
essential simplicity of the fundamental abstraction (monads, anyone?)
...
What do you need to know to follow the article?  Not much, but still a
bit:
...
We'll split the code into three files:
 ...
 - And a test suite in a shell script =wyag-tests=.
We're going to organize our code in a very simple way: we'll have an
executable, =wyag=, a library, =libwyag.py=, and a test suite in a
separate executable  To create the binary, open a new
file called =wyag=, insert the following contents:
...
#+BEGIN_SRC shell :exports none
  ...
  # the article for more information. <http://wyag.thb.lt/#contributing>  #
  #########################################################################

#+END_SRC

** Boilerplate, 1: the binary

Create a new file called =wyag= in your text editor, and copy the following few lines:

#+BEGIN_SRC python :tangle wyag :noweb strip-export
  #!/usr/bin/env python3
  # <<file_header>>
  import libwyag
  libwyag.main()
...
** Boilerplate, 2: the library

The library must be called =libwyag.py=, and reside in the same directory
as the =wyag= executable.  Create =libwyag.py=.

We're going to need a bunch of imports
 ...
 - Git is a CLI application, so we'll need something to parse
   ...
   #+BEGIN_SRC python :tangle libwyag.py :noweb strip-export
   #<<file_header>>
   import argparse
   #+END_SRC
...
Now we have all our imports, let's begin by creating the command-line
arguments parser, using =argparse=.  It's a nice library, but its
interface may not be the most intuitive ever; if need, refer to its [[https://docs.python.org/3/library/argparse.html][documentation]].
...
will actually /require/ one --- just like you don't just call =git=, you
call =git COMMAND=.
...
#   I'm glad you pointed that out!  Yes, the long sequence of =elif= isn't
#   too elegant, but it has a strong advantage: it doesn't require that
#   all commands are actually defined to run.  The more pythonic
#   approach of using a dict would break until the program is complete,
#   because you can't define a dict with undefined values (none of the
#   =cmd_*= functions exist yet!
# #+END_note

** boilerplate, 3: the test suite
...
exact same arguments and compare the results.

#+BEGIN_SRC shell :tangle wyag-tests :noweb strip-export
  #!/usr/bin/env bash
  #<<file_header>>
  LEFT=$(pwd)/wyag
  RIGHT=git
...
repository is a filesystem directory, with a =.git= subdirectory inside
it, and a few other paths which we'll discuss in a moment.
...
We're going to be manipulating a *lots* of paths in repositories.  We
may as well create a few utility functions:
...
section (=[core]=) and three fields:
 ...
 - =repositoryformatversion = 0=: as its name indicates, the version of
   the gitdir format.  0 means the initial format, 1 the same with
   extensions.  If > 1, git will panic.
...
1. We need to create an argparse subparser to handle our command's
   argument.  In the case of =init=, there's a single, optional,
...
To test our new function, we'll call =init= from =git= and =wyag=, with
and without a path argument, and we'll just check that =git status=
behaves exactly the same in the two directories.
...
While we're implementing repositories, we need just a last small
thing: Git functions besides =init= use an existing repository.  It's
often in the current directory, but you may call git from deeper in
the directory.  For example, your repository's root may be in
...
current directory and recursing back until =/=.

To identify something as a repo, it will check for the presence of a
=.git= directory.
...
the actual files git is supposed to version --- source code, for
example.  Commit are stored as objects, too, as well as tags.  With a
few notable exceptions (which we'll see later!), almost everything, in
...
More precisely, Git renders the hash as an hexadecimal string, and
splits it in two parts: the first two characters, and the rest.  It
uses the first part as a directory name, the rest as the file name
(this is because most filesystems hate having too many files in a
single directory and will slow down to a crawl.  Git approaches creaes
256 possible directories, hence dividing the average number of files
per directory by 256)
...
Before we begin implementing the object storage system, we must
understand their exact storage format.  A object start by an header
that specify its type: it may be =blob=, =commit=, =tag= or =tree=.  This
header is followed by a space, then the size of the object in bytes as
an ASCII number, then 0x00 (the null byte), then the contents of the
object.  A commit object begins like this:
...
first line are the start first of that object's contents, the word
...
=.git/objects/.git/objects/e6/73d1b7eaa0aa01b5bc2442d570a765bdaae751=.
...
When all is done, we just call the constructor.
...
The =object_find= function?  It's actually a
...
ways to refer to objects: full hash, short hash, tags...  We won't
implement anything like that for now (only full hashes will be
accepted).  But when we'll want to add more subtle object naming
mechanisms, we'll just have to override =object_find=.

** Writing objects

Writing an object is basically the reverse process: we compute the
hash, insert the header, zlib-compress everything and write the result
in place.  This really shouldn't require much explanation, just notice
...
git is stored as a blob.  That make blobs easy to manipulate, because
...
=git cat-file= is just two positionals: type, and an object identifier:

#+BEGIN_EXAMPLE
  git cat-file TYPE OBJECT
#+END_EXAMPLE

Our subparser is simple enough:
...
And now, we're all set for creating =cat-file=!
...
from either stdin or a named file, and stores it in the repository.
Without the -w option, it doesn't actually perform storage but only
displays the object hash.

The basic syntax of =git hash-object= looks like this:

#+BEGIN_EXAMPLE
  git hash-object [-w] [-t TYPE] [--stdin | FILE]
#+END_EXAMPLE
...
The actual implementation is very simple.  As usual, we create a
small bridge function:
...
* Working with commits, part 1: log
...
The format is actually very simple.  It begins with a series of
key-value pairs, with the first space character as a separator, and ends with the commit message, that may span over multiple lines.

The only special case (I know of) is the =gpgsig= field, which spans on
multiple lines, including an empty line, and will need some special
handling, but nothing too hard.
...
- =tree= is a reference to a tree object, a type of object that we'll
  ...
  describes a state of the work tree state.  Put simply, it's it is
  the actual contents of the commit: files, and where they go.
...
- =author= and =committer= are separate, because the author of a commit
  is not necessarily the person who can commit it (I /think/ this is especially
  useful when working through e-mail, like the Linux kernel does)
- =gpgsig= is the PGP signature of this object.

We'll start by writing a simple parser for the format.  Except for
=gpgsig=, the code is obvious.  The name of the function we're about to
create, =kvlm_parse()=, may be confusing: it isn't called
=commit_parse()= because tags have the very same format, so we'll use it
in both contexts.  KVLM stands here for "Key-Value List with Message".
...
We use an OrderedDict here because =cat-file=, as we've implemented it, will print a commit object by parsing it and re-serializing it!  Also, in Git, the order keys appear in commit and tag object seem to matter.
...
commit or a single file, you've actually created a different commit.
Each and every commit is bound to its place and its relationship to
the whole repository up to the very first commit.  To put it
otherwise, a given commit ID not only identifies some file contents,
but it also binds the commit to its whole history and to the whole
repository.
...
* Working with commits, part 2: checkout
...
- =[mode]= is six bytes and is an ASCII representation of a file mode.
  For example, 100644 is encoded with byte values 49 (ASCII "1"), 48
  (ASCII "0"), 48, 54, 52, 52.

- It's followed by 32, an ASCII space;

- Followed by the path terminated by 0x00;

- Followed by the SHA-1 in binary encoding, on 20 bytes.  Why binary?  God only
  knowns.

The parser is going to be quite simple.  First, create a tiny object wrapper for a tree leaf:
...
Because a tree object is just the repetition of the same fundamental data structure, we write the parser in two times.  First, a parser to extract a single record, which returns the parsed records and the position it reached in input data:
...
Now that we have a parser, we can implement the =GitTree= class:
...
While we're at it, let's add the =ls-tree= to wyag command.  It's
...
Again, we're going to oversimplify the actual git command to make our implementation clear and understandable.  We're also going to add a few safeguards.  Here's how our version of checkout will work:
...
Remember we've created a stupid function, called =object_find=, that
would take four arguments, return the second and ignore the other three?
It's time to replace it by something more useful.  The new
=object_find()= will work like this:

 - it will take a repository as its first argument, and a name as the
   second argument.
 - it will accept an optional format (=fmt=) argument (tree, commit, blob or
   tag).  If given, it will do some dereferencing magic to return the
   hash of an object of *this* type.  This argument would default to
   =None=, which means no format is expected.
 - it will accept a fourth, optional, argument, =follow=, defaulted to
   True.  If follow is =False=, the first hash it finds will be returned
   if it matches =fmt=, or if =fmt= is None, or the function will raise a
   bad object type exception.

Now, what will this new version do:

 - If =name= is HEAD, it will just resolve =.git/HEAD=;
 - If =name= is a hash, and an object with that hash exists, this hash
   is returned.
 - If =name= looks like a short hash, it will look up hashes matching
   it (that is, objects whose hash has the short hash as a prefix);
 - It will try to determine if name identifies a branch or a tag;
...
When this is done, it will count the number of objects it hash
collected: if 0, it will abort (this object cannot be found), if more
than 1, it will abort to (ambiguous reference).  If exactly one, if
=fmt==None=, it is returned.  If =fmt= is provided, we will try to resolve
the reference to a reference of the correct type.  This is actually a
very simple iterative process:

#+CAPTION: Columns are /expected/ types (the value of =fmt=), rows are actual types (the actual object we've found).  An empty cell means no solution.
|        | Blob                   | Commit                    | Tree                           | Tag           |
|        |                        |                           |                                |               |
|--------+------------------------+---------------------------+--------------------------------+---------------|
| Blob   | It's a match!          |                           |                                |               |
| Commit |                        | It's a match!             | Dereference this commit's tree |               |
| Tree   |                        | Return this commit's tree | It's a match!                  |               |
| Tag    | Dereference and repeat | Dereference and repeat    | Dereference and repeat         | It's a match! |
...
following before you start working.
 ...
 - Do /not/ make changes to =wyag=, =libwyag.py= or =wyag-tests=.  These files
...
[fn:2] And there's a use case: imagine you're using git at a very low
level, creating your blobs by hand with =git hash-object= and your trees
with =git mktree=: tagging some object prevent =git gc= from deleting it.

[fn:1] In Terry Pratchett's Discworld, trolls believe they progress in
time from the future to the past.  The reasoning behind that belief is that
